...
SQL;
  _ecobat_run_query($db, $qry, 'ecobat_passes_set_calc_fields');
}
...
GROUP BY taxa_taxon_list_id,
  ...
  site_name;
SQL;
  _ecobat_run_query($db, $qry, 'rollup_passes_into_occurrences insert occurrences');
...
SQL;
  _ecobat_run_query($db, $qry, 'rollup_passes_into_occurrences update passes');
}
...
SQL;
  _ecobat_run_query($db, $qry, '_ecobat_update_reporting_fields');
}
...
function _ecobat_update_map_sq($last_run_date, $db) {
  ...
  $taken = microtime(TRUE) - $tm;
  if ($taken > 10) {
    kohana::log('alert', "_ecobat_update_map_sq took $taken seconds");
  }
}
...
SQL
  ...
  foreach ($occs as $ecobat_occurrence) {
    ...
    if ($thisSample !== $lastSample) {
      ...
      if ($errors = $sample->getAllErrors()) {
        kohana::log('alert', 'Unable to save ecobat sample: ' . json_encode($errors));
		    kohana::log('debug', 'Ecobat sample data: ' . var_export($s, TRUE));
        continue;
      }
      // @todo Error Check
  ...
  $taken = microtime(TRUE) - $tm;
  if ($taken > 10) {
    kohana::log('alert', "_ecobat_insert_occurrences took $taken seconds");
  }
}
...
where eograndparent.id is null
  ...
  and eo.created_on>='$last_run_date'
SQL;
  _ecobat_run_query($db, $qry, '_ecobat_insert_missing_taxonomic_parents');
}
...
SQL;
  _ecobat_run_query($db, $qry, '_ecobat_calculated_summed_passes');
}

/**
 * Runs a query with a timer for diagnostics.
 *
 * @param object $db
 *   Database connection.
 * @param string $qry
 ...
 */
function _ecobat_run_query($db, $qry, $title) {
  $tm = microtime(TRUE);
  $r = $db->query($qry);
  $taken = microtime(TRUE) - $tm;
  if ($taken > 5) {
    kohana::log('debug', "Ecobat query $title took $taken seconds. Updated " . $r->count());
  }
}
